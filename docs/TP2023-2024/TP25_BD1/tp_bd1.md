TP25 : Bases de données - requêtes simples
==
...
8. Quelles sont les communes (tous les attributs) qui ne sont pas
  des chefs-lieux (de département ou de région) ? [34869 lignes]
9. Quels sont les codes et noms des communes dont le chef-lieu
  départemental et le chef-lieu régional ne sont pas les mêmes ?
  [29773 lignes]
10. Donner la table des communes avec comme colonnes : le nom de la
...
11. Donner, sans doublons, le nom des communes qui ne sont pas les
  seules à avoir ce nom au sein d'une même région, dans l'ordre
  lexicographique croissant. [623 lignes]

## Combiner toutes les lignes d'une réponse
Quand la réponse à une requête est une colonne unique, on peut faire
une opération sur cette colonne:
...
* sommer les valeurs des lignes si ce sont des valeurs numériques :

```
SELECT SUM(col) FROM table WHERE ...
```

* prendre la moyenne des valeurs des lignes si ce sont des valeurs numériques :

```
SELECT AVG(col) FROM table WHERE ...
```

* prendre le maximum des valeurs des lignes :

```
SELECT MAX(col) FROM table WHERE ...
```

* prendre le minimum des valeurs des lignes :

```
SELECT MIN(col) FROM table WHERE ...
```

12. Dans combien de régions différentes existe-t-il une commune
  s’appelant _Sainte-Marie_ ?
13. Quel est le taux (nombre flottant entre 0 et 1) de communes dont
...
18. Quelles sont les communes sans aucun habitant ? La table aura
  les mêmes colonnes que la table `communes`. On pourra utiliser
  l'étoile préfixée par le nom ou l'alias de la table. [6 lignes]
19. Quelles sont les 19 communes les moins peuplées, parmi celles
...
20. Donnez les communes (tous les attributs) ayant autant
...
21. Quels sont les codes et noms des départements dans lesquels
...
22. Combien y a-t-il de boulangeries en France ? [48568]
23. Combien y a-t-il, en moyenne, de pharmacies par commune ? [entre
    69% et 70%]
24. Dans combien de communes y-a-t-il strictement plus de
    poissonneries que de crèches ? [572]
25. Quel est le nom de la commune avec le plus faible nombre
