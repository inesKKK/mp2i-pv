# TP8 : listes chaînées et gestion de la mémoire

<!-- Il existe une version sans liste chaînée 2024-2025 -->
...
**Ajoutons une option à la compilation** L'option `-fsanitize=address`
permet d'ajouter des informations sur la mémoire à l'exécution. Il
faut désormais l'ajouter systématiquement.
...
* [serpent.h](tests/serpent.h) qui contient la définition de `struct
  serpent` et de `struct ecaille` et qu'il faudra compléter avec les
  signatures des fonctions à écrire.

Vous devez écrire votre code dans un fichier `serpent.c` (à créer).

**Question 1.** Écrire et documenter un initialisateur `new_serpent` qui
   prend en paramètre une taille et renvoie un pointeur sur un serpent
   de cette taille-là (c'est-à-dire dont le nombre d'écailles est
   la taille, sans compter la tête) dont la tête est à l'abscisse 0 et
   toutes les écailles et la tête sont à l'ordonnée 0. Prévoyez le cas où
   l'utilisateur donnerait une taille négative.


**Question 2.** En fin de vie le serpent disparaît et il faut libérer la
mémoire qui lui est associée. Écrire une fonction `free_serpent` qui
prend en argument un pointeur sur serpent et libère toute la mémoire
associée.


**Question 3.** À chaque étape de sa vie, le serpent avance d'une case
vers la droite, c'est-à-dire que l'abscisse de sa tête augmente
de 1. L'ordonnée de sa tête peut rester identique, augmenter de 1 ou
diminuer de 1. La fonction `rand` déclarée dans `stdlib.h` renvoie un
entier pseudo aléatoire entre 0 et `RAND_MAX` (constante définie dans
`stdlib.h`).

1. Écrire et documenter une fonction récursive `avance_queue` qui prend
  en argument un pointeur sur `struct evaille` et un entier `y` et met
  l'ordonnée de la première écaille de la liste à `y` et décale les
  ordonnées écaille par écaille.

2. Écrire et documenter une fonction `avance` qui prend en argument un
   pointeur sur serpent et fait avancer le serpent (évidemment, c'est
   le même serpent, donc il ne s'agit pas d'en créer un nouveau).


**Question 4.** En vue de pouvoir faire un affichage, on a besoin de
transformer des données sous forme de type structuré serpent en un
tableau bidimensionnel de booléens (l'affichage se fera sur deux
couleurs).

Écrire et documenter une fonction `image` qui prend en argument un
pointeur sur serpent, un entier pour la largeur et un entier pour la
hauteur et renvoie un tableau bidimensionnel aux bonnes dimensions
avec des `false` dans toutes les cases où le serpent ne se trouve pas
et des `true` là où il se trouve. On considèrera que le point de
coordonnées $(0,0)$ se trouve sur le bord de gauche au milieu.

Pour obtenir un gif animé du mouvement du serpent, nous allons
fabriquer un certain nombre de fichiers images au format `ppm`, et
nous demanderons ensuite à `Imagemagick` de fabriquer le gif animé.
...
**Question 5.** Écrire et documenter une fonction `trajet` qui prend en
argument la longueur d'un serpent, la largeur et la hauteur d'une
fenêtre, et permet de créer tous les fichiers d'images correspondant à
chaque étape de l'avancée du serpent.
